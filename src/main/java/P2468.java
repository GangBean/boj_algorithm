    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            min = Math.min(min, Arrays.stream(map[i]).min().orElseThrow());
            max = Math.max(max, Arrays.stream(map[i]).max().orElseThrow());
        }
                    ...
                    if (!isVisited[x][y] && map[x][y] > h) {
                        ...
                        while (!q.isEmpty()) {
                            ...
                            for (int[] dir : DIR) {
                                int nx = cx + dir[0];
                                int ny = cy + dir[1];
                                if (0 <= nx && nx < N && 0 <= ny && ny < N && map[nx][ny] > h && !isVisited[nx][ny]) {

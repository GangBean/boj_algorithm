...
public class P1759 {
    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        findPassword(new ArrayList<>(), letters, L, 0, 0, 0);
        ret.stream().forEach(System.out::println);
    }

    private static void findPassword(List<Character> password, List<Character> letters, int L, int v, int c, int idx) {
        if (password.size() == L) {
            if (v < 1 || c < 2) return;
        ...
        if (letter == 'a' || letter == 'e' || letter == 'i' || letter == 'o' || letter == 'u') {
            password.add(letter);
            findPassword(password, letters, L, v + 1, c, idx + 1);
            password.remove(password.size() - 1);
            findPassword(password, letters, L, v, c, idx + 1);
            return;
        }
        password.add(letter);
        findPassword(password, letters, L, v, c + 1, idx + 1);
        password.remove(password.size() - 1);
        findPassword(password, letters, L, v, c, idx + 1);
    }
}
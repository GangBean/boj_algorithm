...
public class P2630 {
    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        Arrays.stream(countOfPapers(0, 0, N)).mapToObj(String::valueOf).forEach(joiner::add);
        System.out.println(joiner);
    }

    private static int[] countOfPapers(int x, int y, int n) {
        boolean isSame = true;
        for (int i = x; i < x + n; i++) {
            for (int j = y; j < y + n; j++) {
                if (map[x][y] != map[i][j]) {
        ...
        if (isSame) return (map[x][y] == 0) ? new int[]{1, 0} : new int[]{0, 1}; // white, blue

        int[] ret = new int[]{0, 0};
        sum(ret, countOfPapers(x, y, n / 2));
        sum(ret, countOfPapers(x, y + n / 2, n / 2));
        sum(ret, countOfPapers(x + n / 2, y, n / 2));
        sum(ret, countOfPapers(x + n / 2, y + n / 2, n / 2));
        return ret;
    }

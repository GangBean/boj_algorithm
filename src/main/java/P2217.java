...
public class P2217 {
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            lope.add(Integer.parseInt(reader.readLine()));
        }
        Collections.sort(lope, Collections.reverseOrder());

        int ret = lope.get(0);
        for (int i = 1; i < N; i++) {
            lope.set(i, Math.max(lope.get(i-1), lope.get(i) * (i + 1)));
            ret = Math.max(ret, lope.get(i));
        }

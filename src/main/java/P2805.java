...
public class P2805 {
    ...
    *  (1 ≤ N ≤ 1,000,000, 1 ≤ M ≤ 2,000,000,000)둘째 줄에는 나무의 높이가 주어진다.
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        int[] trees = Arrays.stream(reader.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        ...
        while (min < mid && mid < max) {
            long length = lengthOfCutting(trees, mid);
            if (length > M) {
                min = mid;
                mid = (min + max) / 2;
                continue;
    ...
    private static long lengthOfCutting(int[] trees, int mid) {
        long ret = 0;
        for (int tree : trees) {
            ret += Math.max(tree - mid, 0);
        }
        return ret;
    }
}
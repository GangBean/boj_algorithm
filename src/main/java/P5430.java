...
public class P5430 {
    ...
    *  (0 ≤ n ≤ 100,000)다음 줄에는 [x1,.
    * .
    * .
    * ,xn]과 같은 형태로 배열에 들어있는 정수가 주어진다.
    *  (1 ≤ xi ≤ 100)전체 테스트 케이스에 주어지는 p의 길이의 합과 n의 합은 70만을 넘지 않는다.
    * 
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        while (T-- > 0) {
            ...
            for (int i = 0; i < N; i++) {
                nums.add(Integer.valueOf(split[i]));
            }
            ...
            if (!isError) {
                ...
                if (reverse) {
                    for (int i = nums.size() - 1; i >= 0; i--) {
                        tmp.add(String.valueOf(nums.get(i)));
                    }
                ...
                joiner.add(tmp.length() == 0 ? "[]" : tmp.toString());
            }
        }
